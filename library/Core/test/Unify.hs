...
shouldHaveUnsolved name g s t p =
  T.testCase ("⋯ " <> name) $
    case unifyV g s t of
      ...
      Right (Success σ u) ->
        unless (p u) $ assertFailure $
          "unsolved: " <> show u <> "\n" <>
          "     lhs: " <> show (appSubstV σ s) <> "\n" <>
          "     lhs: " <> show (appSubstV σ t) <> "\n" <>
          "       σ: " <> show σ
...
shouldError name g s t p =
  T.testCase ("✗ " <> name) $
    case unifyV g s t of
      ...
      Right (Success σ u) -> assertFailure $
          "unexpected success\n" <>
          "unsolved: " <> show u <> "\n" <>
          "     lhs: " <> show (appSubstV σ s) <> "\n" <>
          "     lhs: " <> show (appSubstV σ t) <> "\n" <>
...
coreUnifier =
  T.testGroup "unifier" [
    shouldUnifyTo "A ≟ A"
      globals (free "A") (free "A") (free "A"),
    shouldError "A ≟ B"
      globals (free "A") (free "B") isClash,
    shouldUnifyTo "α ≟ A"
      globals (VMeta α) (free "A") (free "A"),
    shouldHaveUnsolved "α ≟ α"
      globals (VMeta α) (VMeta α) (== [α]),
    shouldHaveUnsolved "α ≟ β"
      globals (VMeta α) (VMeta β) (not . Meta.nullS),
    shouldError "α ≟ F α"
      globals (VMeta α) (appF (VMeta α)) isOccurs,
    shouldUnifyTo "⋆₀ ≟ ⋆₀"
      globals (VStar 0) (VStar 0) (VStar 0),
    shouldError "⋆₂ ≟ ⋆₁"
      globals (VStar 2) (VStar 1) isClash,
    shouldUnifyTo "ℕ ≟ ℕ"
      globals (VPrimTy Nat.Ty) (VPrimTy Nat.Ty) (VPrimTy Nat.Ty),
    shouldUnifyTo "1 ≟ 1"
      globals nat1 nat1 nat1,
    shouldError "1 ≟ 2"
      globals nat1 nat2 isClash,
    shouldUnifyTo "Π(0 x: A). B ≟ Π(0 x: A). B"
      globals π₀AB π₀AB π₀AB,
    shouldError "Π(1 x: A). B ≟ Π(0 x: A). B"
      globals π₁AB π₀AB isClashU,
    shouldUnifyTo "Π(1 x: α). B ≟ Π(1 x: A). β"
      globals π₁αB π₁Aβ π₁AB,
    shouldError "Π(1 x: α). B ≟ α"
      globals π₁αB (VMeta α) isOccurs,
    shouldUnifyTo "K α ≟ K 1  -- inj"
      globals (appK (VMeta α)) (appK nat1) (appK nat1),
    shouldError "F α ≟ F 1  -- not inj"
      globals (appF (VMeta α)) (appF nat1) isClash
  ]
  where
    globals =
      -- K injective, F not
